    ...
    , ui(new Ui::TcpClient)
{
   ...
   // resize(500,250);
    loadConfig();//输出打印
    connect(&m_tcpSocket,SIGNAL(connected()),this,SLOT(showConnect()));//关联
...
void TcpClient::recvMsg()//注册回复
{
  ...
  switch(pdu->uiMsgType)
  {
    ...
    PDU *respdu = mkPDU(0);//回复的
    memcpy(respdu->caData, pdu->caData, 64);//放进去
...
void TcpClient::on_send_pd_clicked()
{
        ...
        {
            QMessageBox::warning(this,"信息发送","发送的信息内容不能为空");
        }
}
#endif
