...
void OpeDB::init()
{
    ...
    m_db.setDatabaseName("C:\\Qt\\TcpServer\\cloud.db");
    if(m_db.open())
    {
        QSqlQuery query;//查询数据库
...
bool OpeDB::handleRegist(const char *name, const char *pwd)
{
    ...
    /*char caQuery[128] = {'\0'};
    sprintf(caQuery,"");*///c里面的

    if(NULL == name || NULL == pwd)//考虑形参的有效性
    {
...
bool OpeDB::handleLogin(const char *name, const char *pwd)
{
    ...
    QString data = QString("select * from usrInfo where name=\'%1\' and pwd = \'%2\' and online = 0").arg(name).arg(pwd);
     ...
     QSqlQuery query;//查询数据库
     query.exec(data);
     if(query.next())//获取数据消息,登录成功online修改
     {
         //更新
...
QStringList OpeDB::handleFlushFriend(const char *name)
{
    QStringList strFriendList;
        ...
        QString data = QString("select id from usrInfo where online=1 and id in (select id from friend where friendId = (select id from usrInfo where name=\'%1\'))").arg(name);
        QSqlQuery query;
        query.exec(data);
        while (query.next())
        {
        ...
        data = QString("select id from usrInfo where online=1 and id in (select friendId from friend where id=(select id from usrInfo where name=\'%1\'))").arg(name);
        query.exec(data);
        while (query.next())
        {
            strFriendList.append(query.value(0).toString());
